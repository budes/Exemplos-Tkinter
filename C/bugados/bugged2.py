...
class JogoDaCobrinha:
    ...
    def esquerda(self, event): self.modx, self.mody = -VELOCIDADE, 0; self.VelocidadePlayer()
    def direita(self, event): self.modx, self.mody = VELOCIDADE, 0; self.VelocidadePlayer()
    def cima(self, event): self.modx, self.mody = 0, -VELOCIDADE; self.VelocidadePlayer()
    def baixo(self, event): self.modx, self.mody = 0, VELOCIDADE; self.VelocidadePlayer()
    ...
    def ChecaPontos(self):
        ...
        if colis_maca[:] == colis_player[:]:
            ...
            self.cobra.append([self.canvas.create_rectangle((colis_ponta[0]+1, colis_ponta[1]+1),
                                                            (colis_ponta[2]-1, colis_ponta[3]-1), outline='white',
                                                            fill='white'), 0, 0])
